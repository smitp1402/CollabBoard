...
describe("useBoardObjects", () => {
  ...
  it("sets lastObjectSyncLatencyMs after a write and snapshot", async () => {
    const initialSnapshot = createMockSnapshot([
      {
        id: "sticky-1",
        data: () => ({ type: "sticky", x: 0, y: 0, width: 120, height: 80, text: "Note" }),
      },
    ]);
    ...
    expect(result.current.lastObjectSyncLatencyMs).toBe(null);
    ...
    await act(() => {
      const snapshot = createMockSnapshot([
        { id: "sticky-1", data: () => ({ type: "sticky", x: 0, y: 0, width: 120, height: 80, text: "Note" }) },
        { id: "sticky-2", data: () => ({ type: "sticky", x: 100, y: 100, width: 120, height: 80, text: "New" }) },
      ]);
      snapshotCallback?.(snapshot);
    });

    expect(result.current.lastObjectSyncLatencyMs).not.toBe(null);
    expect(typeof result.current.lastObjectSyncLatencyMs).toBe("number");
    expect(result.current.lastObjectSyncLatencyMs).toBeGreaterThanOrEqual(0);
  });
