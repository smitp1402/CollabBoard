...
describe("usePresence", () => {
  ...
  it("sets lastCursorSyncLatencyMs when cursor write is echoed in cursors snapshot", async () => {
    const user = createMockUser("user1", "Alice");
    const { result } = renderHook(() =>
      usePresence("default", user, { x: 5, y: 10 })
    );

    expect(result.current.lastCursorSyncLatencyMs).toBe(null);
    ...
    expect(result.current.lastCursorSyncLatencyMs).not.toBe(null);
    expect(typeof result.current.lastCursorSyncLatencyMs).toBe("number");
    expect(result.current.lastCursorSyncLatencyMs).toBeGreaterThanOrEqual(0);
  });
